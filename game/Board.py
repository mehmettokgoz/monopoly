        ...
        if self.curr_user == -1:
            DSA = json.dumps(
                {"users": user_serialized, "user_positions": self.user_positions, "user_amounts": self.user_amounts,
                 "cells": self.cells, "current_user": "", "options": self.curr_options, "game_over": self.game_over,
                 "curr_chance_card": self.curr_chance_card, "started": self.is_started})
            return DSA
        else:
            ads = json.dumps(
                {"users": user_serialized, "user_positions": self.user_positions, "user_amounts": self.user_amounts,
                 "cells": self.cells, "current_user": self.users[self.curr_user].username, "options": self.curr_options,
                 "game_over": self.game_over, "curr_chance_card": self.curr_chance_card, "started": self.is_started})
            return ads
