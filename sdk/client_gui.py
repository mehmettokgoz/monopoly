...
class GUI:
    ...
    def update_window(self):
        ...
        tk.Label(self.below_frame, text="command: ", background="black").grid(row=0, column=0)
        self.entry = tk.Entry(self.below_frame, background="black")
        self.entry.grid(row=0, column=1)
        self.entry.bind('<Return>', self.handle_enter)
        tk.Button(self.below_frame, text='send', background="black", command=self.show_entry_fields).grid(row=0,
                                                                                                          column=2,
                                                                                                          stick=tk.W,
                                                                                                          pady=3)
